package com.huanghuai.djt.dongjitang.CustomUI;
...
public class SimpleCycleViewPager extends ViewPager {
    ...
    private HomeFragmentRecycleViewPagerAdapter pagerAdapter;
    ...
    public void setDatasource(List<String> data) {
        ...
        pagerAdapter = new HomeFragmentRecycleViewPagerAdapter(data);
    ...
    private class HomeFragmentRecycleViewPagerAdapter extends PagerAdapter {

        private final ArrayList<ImageView> viewList;

        public HomeFragmentRecycleViewPagerAdapter(List<String> data) {
            if (data == null) {
            ...
            imageView.setImageURI(data.get(data.size() - 1));
            imageView.setScaleType(ImageView.ScaleType.FIT_XY);
            viewList.add(imageView);

            for (int i = 0; i < data.size(); i++) {
                imageView = new SimpleDraweeView(getContext());
                imageView.setImageURI(data.get(i));
                imageView.setScaleType(ImageView.ScaleType.FIT_XY);
                viewList.add(imageView);
            }

            imageView = new SimpleDraweeView(getContext());
            imageView.setImageURI(data.get(0));
            imageView.setScaleType(ImageView.ScaleType.FIT_XY);
            viewList.add(imageView);
            myHandler.sendEmptyMessage(MyHandler.MESSAGE_CHECK);
        }
        @Override
